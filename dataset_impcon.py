...
import pandas as pd
from collate_fn_vn import  collate_fn_ViIHSD
import numpy as np
import random
...
class ViIHSD_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training and self.w_aug:
            item["text"] = self.data["tokenized_text"][index]
        else:
            item["text"] = torch.LongTensor(self.data["tokenized_text"][index])
...
class ihc_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training and self.w_aug:
            item["text"] = self.data["tokenized_post"][index]
        else:
            item["text"] = torch.LongTensor(self.data["tokenized_post"][index])
...
def get_dataloader(train_batch_size,eval_batch_size,dataset,seed=None,w_aug=True,w_double=False,label_list=None,cls_tokens=None, model_type="bert-base-uncased"):
    if model_type=="/data2/npl/ICEK/License-Plate-Detection-Pipeline-with-Experiment-Tracking/assets/phobert-base" or model_type=="bert-custom" or model_type=="HateBERT":
        if w_aug:
            if w_double:
    ...
    elif "ViIHSD" in dataset:
        train_dataset = ViIHSD_dataset(data["train"],training=True,w_aug=w_aug)
        valid_dataset = ViIHSD_dataset(data["val"],training=False,w_aug=w_aug)
        test_dataset = ViIHSD_dataset(data["test"],training=False,w_aug=w_aug)
